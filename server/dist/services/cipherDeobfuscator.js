...
exports.deobfuscateSignature = deobfuscateSignature;
exports.transformNParamInUrl = transformNParamInUrl;
exports.forceReloadPlayer = forceReloadPlayer;
...
let signatureTimestamp = 20613; // Fallback default
// Helpers for caching
function getCacheFilePath(hash) {
    return path_1.default.join(CACHE_DIR, `player_${hash}.js`);
}
function getMetaFilePath() {
    return path_1.default.join(CACHE_DIR, `meta.json`);
}
async function fetchPlayerHash() {
    try {
...
async function downloadPlayerJs(hash) {
...
function extractSignatureTimestamp(playerJs, hash) {
...
function getHardcodedConfig(hash) {
    return KNOWN_PLAYER_CONFIGS[hash] || null;
}
function extractSigFunctionInfo(playerJs, hash) {
...
function extractNFunctionInfo(playerJs, hash) {
...
async function initDeobfuscator(forceRefresh = false) {
    ...
    if (!forceRefresh && fs_1.default.existsSync(metaPath)) {
        try {
            ...
            if (meta && meta.hash && meta.timestamp && (Date.now() - meta.timestamp < CACHE_TTL_MS)) {
                ...
                if (fs_1.default.existsSync(cacheFilePath)) {
                    hash = meta.hash;
                    cachedJs = fs_1.default.readFileSync(cacheFilePath, 'utf8');
    ...
    if (!cachedJs) {
        ...
        if (!forceRefresh && fs_1.default.existsSync(cacheFilePath)) {
            cachedJs = fs_1.default.readFileSync(cacheFilePath, 'utf8');
            signatureTimestamp = 20613;
            ...
            fs_1.default.writeFileSync(cacheFilePath, cachedJs, 'utf8');
        }
    }
    currentHash = hash;
    ...
    if (sigInfo) {
        if (sigInfo.jsExpression) {
            ...
            const sigConstArgs = sigInfo.constantArgs || (sigInfo.constantArg !== null ? [sigInfo.constantArg] : null);
            const preprocessFunc = sigInfo.preprocessFunc;
            const preprocessArgs = sigInfo.preprocessArgs;
    ...
    if (nFuncInfo) {
        if (nFuncInfo.jsExpression) {
            ...
            const nConstArgs = nFuncInfo.constantArgs;
    ...
    const contextObject = {
        ...
        _yt_player: {},
        XMLHttpRequest: Object.assign(function () { }, {
            prototype: {
    ...
    const script = new vm_1.default.Script(modifiedJs);
...
function modifiedJsText(cachedJs, injectionPoint, exportCode) {
...
async function deobfuscateSignature(signatureCipher, videoId) {
    await initDeobfuscator();
    if (!activeContext) {
...
async function transformNParamInUrl(url) {
    await initDeobfuscator();
    if (!activeContext) {
        throw new Error('Deobfuscator context is not initialized');
    }
...
async function forceReloadPlayer() {
