...
const yt_1 = __importDefault(require("./routes/yt"));
...
app.use('/api/covers', (req, res, next) => {
    ...
    next();
}, express_1.default.static(uploadsCoversDir));
// Route bindings
app.use('/api/search', generalLimiter, search_1.default);
app.use('/api/upload', uploadLimiter, upload_1.default);
app.use('/api/stream', stream_1.default); // streaming route (no rate limiting to support seeking and Range requests)
app.use('/api/download', download_1.default); // downloading route (no rate limiting to support large file downloads)
// Exempt streaming/downloading endpoints under /api/yt from rate limiting
const ytLimiter = (req, res, next) => {
...
app.use('/api/yt', ytLimiter, yt_1.default);
app.use('/api/lyrics', generalLimiter, lyrics_1.default);
// Exempt streaming/SSE endpoints under /api/downloads from rate limiting
const downloadsLimiter = (req, res, next) => {
...
app.get('/api/proxy-image', (req, res) => {
    ...
    try {
        ...
        const proxyReq = httpLib.request(imageUrl, (proxyRes) => {
...
app.get('/api/health', (req, res) => {
    res.json({
        ...
        processPool: {
            active: processPool_1.ytdlpPool.getActiveCount(),
            queued: processPool_1.ytdlpPool.getQueuedCount(),
        }
    });
});
...
app.listen(PORT, async () => {
    ...
    (0, youtubeService_1.preWarmClient)();
    (0, customInnertube_1.checkCookieHealth)().catch(() => { });
    ...
    if (PUBLIC_URL) {
        ...
        setInterval(() => {
            ...
            const clientLib = healthUrl.startsWith('https') ? https_1.default : http_1.default;
